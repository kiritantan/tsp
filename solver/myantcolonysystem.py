...
class MyAntColonySystem(BaseSolver):
    ...
    def __init__(self):
        ...
        self.Q = 1.0
        self.ANT_NUMBER = 50
        self.not_work_ratio = 0.2
        self.MAX_ITER = 100
        self.initial_pheromone = 0.1

    def solve(self, city_list):
        start = self.start_time()
        self.initial_pheromone = 1 / (len(city_list) * NearestNeighbor().solve(city_list)[1])
    ...
    def create_ants(self, ground):
        ants = []
        for _ in range(int(self.ANT_NUMBER*self.not_work_ratio)):
            ants.append(Ant(ground, random.randrange(len(ground.city_list)), self.ALPHA, self.BETA, self.initial_pheromone, self.DETERMINISTIC_PROBABILTITY, False))
        for _ in range(int(self.ANT_NUMBER*(1 - self.not_work_ratio))):
            ants.append(Ant(ground, random.randrange(len(ground.city_list)), self.ALPHA, self.BETA, self.initial_pheromone,
                      self.DETERMINISTIC_PROBABILTITY, True))
        return ants
...
class Ant(object):
    def __init__(self, ground, initial_city, alpha, beta, initial_pheromone, deterministic_probability, is_work):
        super().__init__()
        self.RHO_L = 0.9
    ...
    def select_next_city(self, unreached_city_list):
        ...
        if self.is_work:
            ...
            for city in unreached_city_list:
                costs.append(self.get_tract_value_without_pheromone(self.current_city, city))
    ...
    def get_tract_value_without_pheromone(self, source_city, destination_city):
        cost = self.ground.city_cost_matrix[source_city][destination_city]
        return math.pow(1.0/cost, self.beta)
